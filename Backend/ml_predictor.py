...
def predict_performance(features: dict) -> str:
    ...
    try:
        ...
        print("[ML Predictor] Model not found or error:", e, ". Using rule-based fallback.")
...
def _apply_sanity_override(features: dict, predicted_label: str) -> str:
    ...
    The model weighs `previous_avg` heavily, which can cause a near-perfect
    current attempt to be downgraded, or a very poor current attempt to be
    inflated, based on the employee's past history. This override only
    kicks in for clear-cut extremes on THIS attempt; the model's prediction
    is left untouched for the ambiguous middle range.
    """
